...
def GenerateMatrix(folderpath):
    ...
    # Load
    pixel_axon_dir = np.load('.\\Pertubation_Temp\\'+folderpath+'_Axon.npy', allow_pickle=True).item()
    pixel_dendrite_dir = np.load('.\\Pertubation_Temp\\'+folderpath+'_Dendrite.npy', allow_pickle=True).item()
    pixel_bouton_dir = np.load('.\\Pertubation_Temp\\'+folderpath+'_Bouton.npy', allow_pickle=True).item()

    cube_map = np.load('.\\Pertubation_Temp\\'+folderpath+'_Cube_Map.npy', allow_pickle=True).item()
    cube_dict = np.load('.\\Pertubation_Temp\\'+folderpath+'_Cube_Dict.npy', allow_pickle=True).item()
    ...
    # 计算两两文件之间的重叠序列
    connectmap=[]
    for root, dirs, files in os.walk('.\\Pertubation\\'+folderpath):
        for f1 in files:
            for f2 in files:
                if f1==f2:
                    continue
    ...
    for i in range(0,len(connectmap)):
        ...
        if from_area in pixel_bouton_dir.keys():
            ...
            for k in connectmap[i][2]:
                ...
                # axon_l=cube_dict[from_area+'_'+k][0]
                # count+=axon_l*bouton_density_p*dendrite_l/dendrite_p #计算bouton density
            # print([from_area,count])
            if count>0:
                temp=0
                temp=connectmap[i]
                temp.append(count)
                connectmap_new.append(temp)
    np.save('.\\Pertubation_Result\\'+folderpath+'_Bouton.npy',np.array(connectmap_new,dtype=list))
