...
def ScaleAddDelete(par):
    ...
    if ratio < 0: # 要删减
        ...
        if add_num>=len(bouton_pair):# 直接先加一轮
        ...
        for i in range(0,len(add_list)):
            ...
            if add_list[i]>2:
                ...
                f.close()
                continue
            if len(bouton_pair[i])==3: #不管加一个点还是两个点，都只加一个点
                add_id=bouton_pair[i][1]
                data[add_id,1]=5
            elif len(bouton_pair[i])==2: #补充一个点or两个点
                # pass
                linenum=len(data)+1
                if add_list[i]==1:
                    t1=bouton_pair[i][0]
                    ...
                    data=np.r_[data,temp]
            elif len(bouton_pair[i])>3: #选择中间的两个or一个点加上去就行了
                if add_list[i]==1:
...
def run__pool():  # main process
    ...
    for folder in folderlist:
        t=folder.split('_')
        if float(t[1])>0.4: #只有放大的才需要resample
