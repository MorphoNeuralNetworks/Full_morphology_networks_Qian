## 提取一个文件中Axon Segment的距离
import navis,os,math,csv,shutil
import matplotlib.pyplot as plt 
import numpy as np
import common 
def AxonTMDAnalysis(par): # 得补0后的文件
    ...
    for i in range(0,len(contents)):
        ...
        t1=data[i]
        t2=data[int(t1[6])-1]
        data[int(t1[6])-1,9]=data[int(t1[6])-1,9]+1
    ...
    for k in range(len(data)):
        ...
        if i[0]==0:
            continue
        if i[9]==0 or i[9]>1 and i[1]!=3 and i[1]!=4:
            mark=0
            length=0
            t=i
            tt=t
            while tt[6]!=-1:
                t=tt
                length+=t[7]
                if t[9]>1 and t[0]!=i[0] and mark==0:
                    segment_pair[int(i[0])]=int(t[0])
                    mark=1
                tt=data[int(t[6])-1,:]
            data[k,8]=length
    res=[[data[key-1,8],data[segment_pair[key]-1,8]] for key in segment_pair.keys()]
...
def run__pool():  # main process
    ...
    with Pool(cpu_worker_num) as p:
        p.map(AxonTMDAnalysis, par_list)
...
def compute_distances(par):
    ...
    f=open('./TMD_Axon_dis/'+name1+'-'+name2+'.txt','w+')
    f.write(str(dist))
    f.close()
...
if __name__ =='__main__':
    ...
    #             with open('./TMD_Axon_dis/'+name1+'-'+name2+'.txt') as file_object:
    #                 contents = file_object.readlines()
    #                 file_object.close()
    #             t=float(contents[0])
    #             heatmap[i,j]=t
    #             heatmap[j,i]=t
    # np.save("heatmap_axon.npy",heatmap)
    heatmap=np.load('heatmap_axon.npy',allow_pickle=True)
    ...
    # plt.savefig('TMD axon distance.png', dpi=300)
